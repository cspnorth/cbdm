...
(function ($) {
	...
	function builder_progress_bar_init() {
            
            function builder_progress_bar() {
                    $('.module.module-progressbar .tf-progress-bar').each(function(){
                            var $this = $( this );
                            $(this).waypoint(function(){
                                    var bar = $this.find( '.tf-progress-bar-bg' );
                                    bar.width( bar.data( 'percent' ) + '%' );

                                    if( $this.find( '.tf-progress-tooltip' ).length > 0 ) {
                                            var $countup = $this.find( '.tf-progress-tooltip' ),
                                                    id = $countup.attr( 'id' ),
                                                    to = $countup.data( 'to' ),
                                                    decimals = $countup.data( 'decimals' ),
                                                    suffix = $countup.data( 'suffix' );
                                            new countUp( id, 0, bar.data( 'percent' ), decimals, 3, { useEasing : true, decimal : '.', suffix: suffix } ).start();
                                    }
                            }, {
                                    offset: '100%',
                                    triggerOnce: true
            ...
            if($('.module.module-progressbar .tf-progress-bar').length>0){
	...
	$(document).ready(builder_progress_bar_init);
	$( 'body' ).on( 'builder_load_module_partial builder_toggle_frontend', builder_progress_bar_init );
}(jQuery));