 ...
 * @return array
 */
function themify_theme_portfolio_appearance_meta_box() {
    return array(
		// Header Design
		array(
			'name'        => 'header_design',
...
function themify_theme_portfolio_meta_box($args = array()) {
    ...
    return array(
        ...
        array(
                'name'        => 'tile_layout',
                'title'       => __( 'Block Size', 'themify' ),
                'description' => __( 'Select larger block size to highlight this project on archive view.', 'themify' ),
                'type'        => 'layout',
                'show_title'  => true,
                'meta'        => array(
                        array(
                                'value'    => 'size-large image-left',
                                'img'      => 'images/layout-icons/large-image-left.png',
                                'title'    => __( 'Large Size', 'themify' )
                        ),
                        array(
                                'value' => 'size-small image-left',
                                'img'   => 'images/layout-icons/small-image-left.png',
                                'title' => __( 'Regular Size', 'themify' ),
                                'selected' => true
                        ),
                ),
                'default' => 'size-small image-left'
        ),
        // Post Image
        array(
            'name' => 'post_image',
    ...
    class Themify_Portfolio {
        ...
        function updated_messages($messages) {
            ...
            $messages[$this->post_type] = array(
                ...
                9 => sprintf(__('%s scheduled for: <strong>%s</strong>.', 'themify'), $post_type_object->labels->name, date_i18n(__('M j, Y @ G:i', 'themify'), strtotime($post->post_date))),
        ...
        function init_shortcode($atts) {
            ...
            $this->atts = array(
                ...
                'unlink_title' => '',
                'load_more' => 'no',
                'is_portfolio'=>'yes',//only need to understand what shortcode(themify_portfolio_posts or themify_list_posts) to show for multiply infinity scroll
                'offset' => '0',
                'image' => 'yes', // no
                'image_w' => 290,
                'image_h' => 290,
        ...
        function shortcode($atts = array(), $post_type) {
            ...
            $posts = $portfolio_query->query(apply_filters('themify_' . $post_type . '_shortcode_args', $args, $atts));
            ...
            if ($posts) {
                ...
                if (('yes' === $filter || $post_filter !== 'no') && false === stripos($style, 'slider')) {
                ...
                if ('slider' === $style) {
                    ...
                    $out .= themify_get_shortcode_template($posts, 'includes/loop', 'index');
                    $out .= '</div>';
                } else {
...
if (!function_exists('themify_default_portfolio_index_layout')) {
	...
	$output .= '	<input type="hidden" name="' . esc_attr( $prefix ) . 'post_layout" class="val" value="' . esc_attr( $val ) . '" />
        ...
        $output .= '<p>
						<span class="label">' . __('Unlink Portfolio Image', 'themify') . '</span>
						<select name="' . esc_attr($prefix . 'unlink_post_image') . '">' . themify_options_module($default_options, $prefix . 'unlink_post_image', true, 'no') . '
						</select>
					</p>';
        /**
         * Image Dimensions
